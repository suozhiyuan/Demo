		...
		SAssignNew(CSBox, SHorizontalBox)
			...
			[
				...
				[
					...
					.WrappingPolicy(ETextWrappingPolicy::AllowPerCharacterWrapping)		// 填充规则，多行显示
...
void SDemoChatShowWidget::Construct(const FArguments& InArgs)
{
	//获取GameStyle
...
void SDemoChatShowWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	// 临时序列
...
void SDemoChatShowWidget::AddMessage(FText NewName, FText NewContent)
{
...
void SDemoChatShowWidget::InitlizeItem()
{
	for (int i = 0; i < 10; ++i) 
	{
		UnActiveList.Add(MakeShareable(new ChatShowItem(&GameStyle->EmptyBrush, GameStyle->Font_20)));
	}
}
