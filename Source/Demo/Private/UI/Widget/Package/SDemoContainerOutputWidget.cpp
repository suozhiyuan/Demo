...
void SDemoContainerOutputWidget::LeftOperate(int InputID, int InputNum, int& OutputID, int& OutputNum)
{
	...
	{
		OutputID = ObjectIndex;			// 本地物品替换到输出物品
		OutputNum = (InputNum + ObjectNum <= 64) ? (InputNum + ObjectNum) : 64;				// 输出数量，最大64
		ObjectNum = (InputNum + ObjectNum <= 64) ? 0 : (InputNum + ObjectNum - 64);			// 本地数量
